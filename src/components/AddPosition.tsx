...
import { apiClient, CreatePositionData } from "@/lib/api";
...
const AddPosition = ({ onSuccess }: AddPositionProps) => {
    ...
    const createPositionMutation = useMutation({
        mutationFn: (data: CreatePositionData) => apiClient.createPosition(data),
        onSuccess: (response) => {
        ...
        onError: (error: any) => {
            const errorMessage = error?.response?.data?.message || "حدث خطأ أثناء إنشاء المنصب";
    ...
    const onSubmit = (data: PositionFormData) => {
        ...
        createPositionMutation.mutate(cleanedData);
    };
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="max-w-2xl">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        أدخل بيانات المنصب الجديد في النموذج أدناه
                    </DialogDescription>
                </DialogHeader>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <div className="flex justify-end gap-2">
                        ...
                        <Button type="submit" disabled={createPositionMutation.isPending}>
                            {createPositionMutation.isPending ? "جاري الحفظ..." : "حفظ المنصب"}
                        </Button>
                    </div>
                </form>
            </DialogContent>
